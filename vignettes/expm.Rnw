...
The \proglang{C} routine is based on a translation
of the corresponding Octave implementation of function \texttt{expm}, \citep{octave},
made by the \CRANpkg{Matrix} package authors, see \cite{Matrix-pkg}.
